...
private:
	...
	ModuleRenderer(const ModuleRenderer& cpy) = delete; // Disallow copying
	void operator=(const ModuleRenderer& cpy) = delete; // Disallow copying
public:
	~ModuleRenderer();
	...
	size_t readStereo(int32_t samplerate, size_t frames, int16_t * left, int16_t * right);
	size_t readStereo(int32_t samplerate, size_t frames, float * left, float * right);
	size_t readQuad(int32_t samplerate, size_t frames, int16_t * left, int16_t * right, int16_t * rear_left, int16_t * rear_right);
	size_t readQuad(int32_t samplerate, size_t frames, float * left, float * right, float * rear_left, float * rear_right);
	size_t readInterleavedStereo(int32_t samplerate, size_t frames, int16_t * interleaved_stereo);
	size_t readInterleavedStereo(int32_t samplerate, size_t frames, float * interleaved_stereo);
	size_t readInterleavedQuad(int32_t samplerate, size_t frames, int16_t * interleaved_quad);
	size_t readInterleavedQuad(int32_t samplerate, size_t frames, float * interleaved_quad);
};
