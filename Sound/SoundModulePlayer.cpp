...
Audio::FrameCount_T ModulePlayer::outputTo(const Audio::Output &dst)
{
	...
	if(dst.interleavingType == Audio::InterleavingType::INTERLEAVED)
	{
		switch (dst.channelCnt) {
			case 1:
				readFrames = mod.readMono(dst.frameRate,dst.frameCnt,dst.dst);
			break;
			case 2:
				readFrames =  mod.readInterleavedStereo(dst.frameRate,dst.frameCnt,dst.dst);
			break;
			case 4:
				readFrames =  mod.readInterleavedQuad(dst.frameRate,dst.frameCnt,dst.dst);
			break;
		default:
			break;
		}
		...
		switch (dst.channelCnt) {
			...
			break;
			case 2:
				readFrames = mod.readStereo(dst.frameRate,dst.frameCnt,&dst.dst[0],&dst.dst[dst.frameCnt]);
			break;
			case 4:
				readFrames = mod.readQuad(dst.frameRate,dst.frameCnt,&dst.dst[0],&dst.dst[dst.frameCnt],&dst.dst[dst.frameCnt*2],&dst.dst[dst.frameCnt*3]);
			break;
		default:
			break;
		}
...
ModulePlayer::Status ModulePlayer::getState() const
{
	return state;
}
...
double ModulePlayer::seek(double seconds, Audio::Seekable::SeekPos whence)
{
	switch (whence) {
		case SeekPos::CUR:
...
double ModulePlayer::tell() const
{
	return mod.getPosition();
}

double ModulePlayer::size() const
{
	return mod.getDuration();
}
